...
**CIA terms.** For confidentiality, integrity, and availability, the Brazilian Portuguese terms are **confidencialidade**, **integridade**, and **disponibilidade**. In Brazil, these principles are commonly referred to as the **CID triad**, using the Portuguese initials. However, the ISMS application uses the existing `C`, `I`, and `A` codes as internal badge identifiers, so these codes remain unchanged in the `pt-BR` locale. The translation therefore localizes the full terms while preserving the application's existing `CIA` identifiers.
